...
\begin{itemize}
	...
	\begin{itemize}
		\item One-sided: sempre giusto quando esce 1 ($\Pr(A(I,Z) = q(I) \mid A(I,Z) = 1) = 1$)

		\item Two-sided: può sbagliare su entrambi gli output ($\Pr (A(I,Z) = q(I)) > 1/2$)
	\end{itemize}
...
\begin{itemize}
	...
	$$ \Pr (\text{fail}) = \Pr (\text{ret } \Gamma^\ast)^M = \dots \leq e^{- \frac{1}{\binom{|V|}{2}} M} = \epsilon$$
