...
Si tratta di un limite superiore al valore del gioco; con $\alpha \in [0,1]$ il valore del gioco non può essere peggio di:
\begin{itemize}
    \item Predire $+1$ con probabilità $\alpha$ quando il label effettivo è $-1$, subendo costo $c^+ = c(+1,-1)$

    \item Oppure, predire $-1$ con probabilità $(1 - \alpha)$ quando il label effettivo è $+1$, subendo $c^- = c (-1, +1)$
\end{itemize}
