...
Vogliamo un algoritmo per scegliere $I_1, \dots, I_T$ tale che per $T \rightarrow \infty$
...
\begin{itemize}
    ...
    \item per $t > 1$, $\ell_2 = (1,0)$, $\ell_3 = (0,1)$, $\ell_4 = (1,0)$, \dots
\end{itemize}
...
\begin{algorithm}
    ...
    \For{$t = 1, \dots, T$}{
        Definire la distribuzione $p_t(i) = \frac{w_t(i)}{W_t}$, dove $W_t = \sum_{j = 1}^K w_t (j)$\;
...
\begin{align*}
	\ln \left(\frac{W_{T+1}}{W_1}\right) \stackrel{(\ast)}{=} \sum_{t = 1}^T \ln \left(\frac{W_{t+1}}{W_t}\right) & \leq \sum_{t = 1}^T \ln \left(1 \underbrace{- \gamma \sum_{i = 1}^K p_t (i) \ell_t (i) + \frac{\gamma^2}{2} \sum_{i = 1}^K p_t (i) \ell_t (i)^2}_{z} \right) \\
...
\end{align*}
Dove $(\ast)$ vale in quanto diventa simile a una somma telescopica ($\ln (W_2/W_1) + \ln (W_3/W_2) + \dots$, lasciando solo primo e ultimo valore). Questo è un upper bound per la variazione del peso totale degli esperti.

D'altra parte, per ogni $k$ fissato
\begin{align*}
	\ln \left(\frac{W_{T+1}}{W_1}\right) \geq \ln \left(\frac{w_{T+1} (k)}{W_1}\right) & = \ln \left(\frac{1 \cdot \exp \left( - \gamma \sum_{t = 1}^T \ell_T (k) \right)}{\sum_{i = 1}^K 1} \right) \\
	& = - \gamma \sum_{t = 1}^T \ell_T (k) - \ln (K) 
\end{align*}
In altre parole, sicuramente la somma di tutti i pesi ($W_{T+1}$) sarà maggiore o uguale al peso di un singolo esperto ($w_{T+i} (k)$). Questo è un lower bound per la variazione dei pesi.
