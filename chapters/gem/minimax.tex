...
Consideriamo un gioco come la morra cinese (carta, forbice, sasso) oppure pari-e-dispari, dove il giocatore e il suo avversario rivelano simultaneamente le proprie mosse. Questi giochi sono detti a somma zero, in quanto ciò che vince il giocatore lo perde l'avversario e viceversa; possiamo rappresentarli con una matrice reale $G$ di dimensioni $m \times n$, dove le $m$ righe rappresentano le mosse del giocare e le $n$ colonne rappresentano le mosse dell'avversario. Se il giocatore sceglie la mossa $i$ e l'avversario sceglie la mossa $j$, allora il giocatore guadagna $G_{i,j}$ e l'avversario perde $- G_{i,j}$. Se $G_{i,j} > 0$, allora l'avversario paga $G_{i,j}$ al giocatore; se invece $G_{i,j} < 0$ è il giocatore a pagare $G_{i,j}$ all'avversario. Per esempio, la matrice $G$ di morra cinese è indicata qui sotto a sinistra
\begin{table}[h]
    \centering
...
dove l'ultimo termine utilizza la notazione matriciale per denotare la doppia somma che lo precede. Sfruttando questa identità, possiamo allora scrivere il guadagno atteso del giocatore come
...
$$ I = \arg \max_{i = 1, \dots, n} \sum_{j = 1} G_{i,j} q_j $$
...
Un altro modo di interpretare il teorema minimax è il seguente. Sia $V_G$ il valore comune delle formula al membro sinistro e destro nell'enunciato del teorema. Allora il giocatore ha una strategia
...
\begin{lemma}
    ...
    Per qualsiasi intero positivo $T$ e per qualsiasi sequenza $\bm q_1, \dots, \bm q_T$ di strategia dell'avversario, l'algoritmo Hedge con parametro $\eta > 0$ garantisce
    $$ \sum_{t = 1}^T \bm p_t^\top G \bm q_t \geq \max_{\bm p} \sum_{t = 1}^T \bm p^\top G \bm q_t - \frac{2 \ln m}{\eta} - \eta T $$
\end{lemma}
\begin{proof}
    Per applicare Hedge, riscaliamo gli elementi di $G$ nell'intervallo $[0,1]$. Definiamo quindi il vettore delle perdite delle azioni al tempo $t$ come $\bm \ell_t = (\bm 1 - G \bm q_t) / 2 \in [0,1]$, dove $\bm 1 = (1, \dots, 1)$. L'analisi di Hedge garantisce che le distribuzioni $\bm p_1, \dots, \bm p_t$ calcolate dall'algoritmo soddisfano
    \[
    \sum_{t = 1}^T \bm \ell_t^\top \bm p_t \leq \min_{i = 1, \dots, m} \sum_{t = 1}^T \ell_t (i) + \frac{\ln m}{\eta} + \frac{\eta}{2}T \tag*{$(\ast \ast)$}
    \]

    Dato che nel simplesso delle probabilità $\left\{\bm p \geq 0: \bm p^\top \bm 1 = 1\right\}$ la funzione lineare
    $$ F(\bm p) = \sum_{t = 1}^T \bm \ell_t^\top \bm p $$
    è minimizzata in un vertice del simplesso, abbiamo che
    $$ \min_{i = 1, \dots, m} \sum_{t = 1}^T \ell_t (i) = \min_{\bm p} \sum_{t = 1}^T \bm \ell_t ^\top \bm p $$

    Ricordando che $\bm \ell_t = (\bm 1 = G \bm q_t)/2$ abbiamo che, per un $\bm p$ arbitrario
    $$ \bm p^\top \bm \ell_t = \bm p^\top \frac{\bm 1 - G \bm q_t}{2} = \frac{\bm 1 - \bm p^\top G \bm q_t}{2} $$
...
\begin{proof}[Dimostrazione di minimax (\ref{theo:minimax})]
    ...
    \begin{align*}
        ...
        & = \min_{\bm q} \frac{1}{T} \sum_{t = 1}^T \bm p_t^\top G \bm q \\
        & \geq \frac{1}{T} \sum_{t = 1}^T \min_{\bm q} \bm p_t^\top G \bm q \\
        & = \frac{1}{T} \sum_{t = 1}^T \bm p_t^\top G \bm q_t \\
        & \geq \max_{\bm p} \frac{1}{T} \sum_{t = 1}^T \bm p^\top G \bm q_t - \frac{2 \ln m}{\eta T} - \eta \\
        & = \max_{\bm p} \bm p^\top G \bar{\bm q} - \frac{2 \ln m}{\eta T} - \eta \\
        & \geq \min_{\bm q} \max_{\bm p} \bm p^\top G \bm q - \frac{2 \ln m}{\eta T} - \eta
    \end{align*}
