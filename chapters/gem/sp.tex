...
\]
ovvero, per ogni valore di $y \in [0, 1 - \delta]$ e $\delta$ positivo, la perdita $\ell_t (y + \delta)$ non può essere minore della perdita $\ell_t (y) - \delta$. Intuitivamente, aggiungere $\delta$ al prezzo di riserva porta a una perdita di almeno $\delta$ in meno.
...
Introduciamo l'algoritmo Exp3-RTB (Real-Time Bidding, Algoritmo \ref{alg:e3-rtb}), una variante di Exp3 che sfrutta il migliore feedback $\left\{\ell_t (y) : y \geq r_t \right\}$. L'algoritmo usa una discretizzazione dello spazio d'asta $[0,1]$ in $K = \lceil 1/\gamma \rceil$ aste $y_k := (k-1) \gamma$ per $k = 1, \dots, K$.
...
\begin{center}
    \begin{algorithm}
        ...
        \For{$t = 1, 2, \dots$}{
            ...
            $$q_t (k) = (1 - \gamma) p_t (k) + \gamma \Ind \left\{k  = 1\right\}$$ 
            per $k = 1, \dots, K$\;
            Estrai $I_t \sim q_t$ e imposta $r_t = (I_t - 1)\gamma$\;
            \For{\textnormal{\textbf{each}} $k = 1, \dots, K$}{
...
\begin{proof}
    ...
    \[ \sum_{t = 1}^T \sum_{i = 1}^K p_t (i) \hat \ell_t (i) - \sum_{t = 1}^T \hat \ell_t (k) \leq \frac{\eta}{2} \sum_{t = 1}^T \sum_{j = 1}^K p_t (j) \hat \ell_t (j)^2 + \frac{\ln (K)}{\eta} \tag*{$(\ddag)$}\]
	...
	Quindi, prendendo il valore atteso da entrambi i lati di $(\ddag)$ implica, similmente a come fatto nell'analisi di Exp3,
    ...
    \begin{align*}
        \sum_{j = 1}^K \frac{q_t (j)}{\sum_{i = 1}^j q_t (i)} & = 1 + \sum_{j = 2}^K \frac{s_t (j) - s_t (j-1)}{s_t(j)} && (\text{def. } s_t(j)) \\ 
        & = 1 + \sum_{j = 2}^K \int_{s_t (j-1)}^{s_t (j)} \frac{1}{s_t(j)} dx && \left(\int_{A}^B C \cdot dx = (B - A) C \right)\\
        & \leq 1 + \sum_{j = 2}^K \int_{s_t (j-1)}^{s_t (j)} \frac{1}{x} dx && \left(x \leq s_t (j) \Leftrightarrow \frac{1}{x} \geq \frac{1}{s_t (j)} \right) \\ 
        & = 1 + \int_{q_t (1)}^{1} \frac{1}{x} dx && \left(\text{unione degli integrali}\right)\\
        & \leq 1 - \ln \left(q_t (1)\right) && (\ln(1) = 0) \\ 
        & \leq 1 + \ln \left(\frac{1}{\gamma}\right) && \left(q_t(1) \geq \gamma \Leftrightarrow \frac{1}{q_t (1)} \leq \frac{1}{\gamma} \right)
    \end{align*}
    dove abbiamo usato $q_t (1) \geq \gamma$ (per definizione di $q_t$). Quindi, sostituendo nel bound precedente, otteniamo
    \begin{align*}
    	\Ex \left[\sum_{t = 1}^T \sum_{i = 1}^K p_t (i) \ell_t (y_i) \right] - \sum_{t = 1}^T \ell_t (y_k) & \leq \frac{\eta}{2 (1 - \gamma)} \sum_{t = 1}^T \left(1 + \ln \left(\frac{1}{\gamma}\right)\right) + \frac{\ln K}{\eta} \\
    	& \leq \frac{\eta}{2 (1 - \gamma)} \sum_{t = 1}^T \ln \left(\frac{e}{\gamma}\right) + \frac{\ln K}{\eta}  && (1 = \ln (e)) \\
    	& \leq \frac{\eta T \ln (e /\gamma)}{2 (1 - \gamma)} + \frac{\ln K}{\eta} && (\ast)
    \end{align*}
    ...
    $$ q_t (k) = \left(1 - \gamma\right) p_t (k) + \gamma $$
    ovvero, con probabilità $(1 - \gamma)$ viene usata la distribuzione $p_t$, mentre con probabilità $\gamma$ usa il prezzo più basso (favorendo "l'esplorazione", scegliere il prezzo più basso permette di calcolare quale sarebbe stato il guadagno/perdita per qualsiasi valore di $r > r_t$).
    
    Abbiamo
    \begin{align*}
        \Ex \left[\sum_{t = 1}^T \ell_t (r_t) \right] - \sum_{t = 1}^T \ell_t (y_k) & = \Ex \left[\sum_{t = 1}^T \left( \left(1 - \gamma\right) \sum_{i = 1}^K p_t (i) \ell_t (y_i) + \gamma \ell_t (y_1) \right)\right] - \sum_{t = 1}^T \ell_t (y_k) \\
        & \stackrel{(1)}{\leq} (1 - \gamma) \Ex \left[\sum_{t = 1}^T \sum_{i = 1}^K p_t (i) \ell_t (y_i) \right] + \gamma T - \sum_{t = 1}^T \ell_t (y_k) \\
        & \stackrel{(\ast)}{\leq} \frac{\eta T \ln (e /\gamma)}{2} + \frac{\ln K}{\eta} + \gamma T && (\ast \ast)
    \end{align*}
   In seguito alla prima uguaglianza
    \begin{itemize}
    	\item $\left(1 - \gamma\right) \sum_{i = 1}^K p_t (i) \ell_t (y_i)$ indica la perdita media dovuta alla strategia $p_t$
    	
    	\item $\gamma \ell_t (y_1)$ indica la perdita dovuta all'esplorazione forzata del prezzo più basso
    	
    	\item $\sum_{t = 1}^T \ell_t (y_k)$ indica la perdita totale del miglior prezzo fisso
    \end{itemize}
    Il passaggio $(1)$ è dato dal fatto che 
    $$ \gamma \sum_{t = 1}^T \ell_t (y_1) \leq \gamma T $$
    in quanto la perdita per ognuno dei $T$ turni di esplorazione è al massimo 1. L'ultima disuguaglianza vale per $(\ast)$. 
    
    Per concludere la dimostrazione, limitiamo superiormente il rimorso contro un qualsiasi valore fissato di $y \in [0,1]$. Dato che esiste $k \in \left\{1, \dots, K\right\}$ tale che $y \in \left[y_k, y_k + \gamma \right]$ (ovvero, qualsiasi valore di $y$ deve essere "vicino" a uno dei valori discretizzati) e dato che ogni $\ell_t$ soddisfa la condizione semi-Lipschitz $(\dag)$, abbiamo che $\ell_t (y) \geq \ell_t (y_k) - \gamma$, quindi $\ell_t (y_k) \leq \ell_t (y) + \gamma$. Questo risulta in
    $$ \min_{k = 1, \dots, K} \Ex \left[\sum_{t = 1}^T \ell_t (y_k) \right] \leq \min_{0 \leq y \leq 1} \sum_{t = 1}^T \ell_t (y) + \gamma T $$
    Questo significa che "accontentarsi" della discretizzazione costa al massimo $\gamma T$ in rimorso addizionale.
...
che, per $\gamma = T^{-1/3}$ e $\eta = T^{-2/3}$ fornisce $R_t = \O \left(T^{2/3}\right)$ ignorando fattori logaritmici, un bound molto peggiore di quello ottenuto tramite Exp3-RTB usando il miglior feedback dato dalla struttura di questo problema.
