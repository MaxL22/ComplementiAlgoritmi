...
con probabilità almeno $1 - \delta$ rispetto all'estrazione di $f \in \F$.
...
Il prodotto scalare $\bm Z_j^\top \bm x$ calcola la lunghezza della proiezione di $\bm x$ su $\bm Z_j$ moltiplicata per la lunghezza di $\bm Z_j$. L'idea è quella di calcolare una sorta di "impronta" molto più piccola del vettore originale (uno scalare al posto che $d$ dimensioni), conservandone le proprietà essenziali, usata per fare i calcoli; viene ripetendo il calcolo su $k$ variabili casuali per minimizzare l'errore.
...
\begin{align*}
    ...
    & = \Ex \left[\sum_{i = 1}^d \left(x_i - x_i'\right)^2 Z_{j,i}^2 \right] + \Ex \left[\sum_{r,s : r \neq s} \left(x_r - x_r'\right) (x_s - x_s') Z_{j,r} Z_{j,s}\right] \\
    & = \sum_{i = 1}^d \left(x_i - x_i'\right)^2 \Ex \left[Z_{j,i}^2\right] + \sum_{r,s = 1}^d \left(x_r - x_r'\right)(x_s - x_s') \Ex \left[Z_{j,r}\right]\Ex[Z_{j,s}] \\
    & = \sum_{i = 1}^d \left(x_i - x_i'\right)^2 \Var\left[Z_{j,i}\right] && (\dag) \\
    & = \sum_{i = 1}^d \left(x_i - x_i' \right)^2 = \|\bm x - \bm x'\|^2
\end{align*}
dove $(\dag)$ vale perché le $Z_{j,i}$ hanno media zero e quindi
...
possiamo equivalentemente dimostrare che $1 - \epsilon \leq \|f(\bm v)\|^2 \leq 1 + \epsilon$ per ogni $\bm v \in \R^d$ tale che $\|v\| = 1$.
...
Quindi le variabili casuali $V_j = \left(\bm Z_j^\top\right)^2$ per $j = 1, \dots, k$ sono i.i.d. con media $\mu = 1$ e $(\ddag)$ può essere riscritta come
...
con probabilità almeno $1 - \delta$ rispetto all'estrazione delle $\left\{Z_{j,i} \mid j = 1, \dots, k, \ \ i = 1, \dots, d \right\}$.
