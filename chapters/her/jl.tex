...
e inoltre
\begin{align*}
    \Ex \left[\left(\bm Z_j^\top \bm v \right)^2 \right] & = \Ex \left[\left(\bm Z_j^\top \bm v\right)^\top \left(\bm Z_j^\top \bm v\right)\right] \\
    & = \Ex \left[ \left(\bm v^\top \bm Z_j\right) \left(\bm Z_j^\top \bm v\right)\right] \\
    & = \bm v^\top \Ex \left[\bm Z_j \bm Z_j^\top \right] \bm v \\
    & = \bm v^\top I \bm v \\
    & = \|\bm v \|^2 = 1
\end{align*}
...
\end{cases}$$
In quanto la distribuzione ha $\Ex\left[Z_{j,i}\right] = 0$ (usato per i valori al di fuori della diagonale di $M$) e $\Var \left[Z_{j,i}\right] = \Ex \left[Z_{j,i}^2\right] = 1$ (valori sulla diagonale).

Quindi le variabili casuali $V_j = \left(\bm Z_j^\top \bm v \right)^2$ per $j = 1, \dots, k$ sono i.i.d. (indipendenti e identicamente distribuite) con media $\mu = 1$ e $(\ddag)$ può essere riscritta come
...
Al posto del coefficiente fisso di 2 come nel Lemma \ref{lemma:c-h}, usiamo la notazione $\O$ per indicare una costante. Possiamo quindi trovare un valore per $k$:
\begin{align*}
    e^{-\O(k\epsilon^2)} \leq \delta & \Longleftrightarrow \ln \left(e^{-\O(k \epsilon^2)} \right) \leq \ln \left(\delta\right) \\
    & \Longleftrightarrow - \O \left(k \epsilon^2\right) \leq \ln \left(\delta\right) \\
    & \Longleftrightarrow \O \left(k \epsilon^2\right) \geq \ln \left(\frac{1}{\delta}\right) \\
    & \Longleftrightarrow k \geq \O \left(\frac{1}{\epsilon^2} \log \frac{1}{\delta}\right)
\end{align*}
