...
\begin{proof}
    \begin{align*}
    	\sum_{T \in \T} \beta_T & \leq \sum_{T \in \T} \beta_T |T \cap \Gamma^\ast| && (|T \cap \Gamma^\ast| \geq 1) \\
        & = \sum_{T \in \T} \beta_T \sum_{e \in \Gamma^\ast} \Ind \left\{e \in T \right\} \\
        & = \sum_{e \in \Gamma^\ast} \sum_{T \in \T(e)} \beta_T \\
    	& \leq \sum_{e \in \Gamma^\ast} 1 && (\text{per ipotesi}) \\
    	& = |\Gamma^\ast | = \Opt
    \end{align*}
\end{proof}
...
\begin{lemma}
    ...
    Per ogni $e \in \E$, $e \in \Gamma_A$ se e solo se esiste una chiamata ricorsiva $r$ e un $T \in \T$ tali che $T \subseteq V_r$, $T \in \T(e)$ e $\pi_r = T \setminus e$.
\end{lemma}
...
\begin{proof}
    Si scelga qualsiasi lato $e$ e sia $r$ l'iterazione in cui almeno uno dei due nodi estremi di $e$ viene rimosso da $V_r$. Allora, KwikCluster fa un errore su $e$ se e solo se $e$ forma un triangolo sgradevole con $\pi_r$ e $\pi_r = T \setminus e$.
...
Si noti che per ogni $e \in \Gamma_A$ e per ogni coppia di $T, T' \in \T(e)$ con $T \neq T'$, $A_T$ e $A_{T'}$ non possono accadere assieme in quanto $e$ può formare un triangolo sgradevole assieme a $\pi_r$ in solo uno dei due casi. Quindi, per un qualsiasi lato $e$
...
dove $\Pr(e \in \Gamma_A | A_T) = \frac{1}{3}$ considerato che, dato $r$ tale che $T \subseteq V_r$ e $\pi_r \in T$, $e$ è sbagliato solo se $\pi_r \in T \setminus e$; ovvero la probabilità che venga scelto come pivot il terzo lato di $T$, fuori da $e$.
