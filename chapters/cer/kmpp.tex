...
\[
\phi(\C, C) = \sum_{\bm x \in C} \| \bm c - \mu_C \|^2, \quad \forall C \in \C, \text{ con } \mu_C \text{ centroide di } C \tag*{$(\dag)$}
\]

Come già visto (Teorema \ref{theo:inapprox-lloyd}) l'algoritmo di Lloyd non ha un fattore di approssimazione massimo in quanto gli outlier non sono considerati nell'inizializzazione dell'algoritmo, nonostante possano rappresentare un costo significativo nella soluzione.
...
\begin{proof}
    ...
    \begin{align*}
        \Ex \left[\phi \left(\C_1, A\right) \mid \bm c_1 \in A\right] & = \frac{1}{|A|} \sum_{\bm a \in A} \left(\sum_{\bm x \in A} \| \bm x - \bm a \|^2 \right) && (\C_1 = \left\{\bm c_1\right\}) \\
        & = \frac{1}{|A|} \sum_{\bm a \in A} \left(|A| \| \bm a - \bm \mu \|^2 + \sum_{\bm x \in A} \| \bm x - \bm \mu \|^2\right) && (\mu \text{ centroide di } A) \\
        & = \sum_{\bm x \in A} \|\bm x - \bm \mu \|^2 +\sum_{\bm a \in A} \| \bm a - \bm \mu \|^2 \\
    ...
    \begin{align*}
        ...
        & \leq \| \bm x - \bm c \| + \| \bm a - \bm x \| && \text{(per disuguaglianza triangolare)}
    \end{align*}

    Sapendo che $(a + b)^2 \leq 2 (a^2 + b^2)$ per ogni $a,b \in \R$ e $\| \bm x - \bm c \|^2 = \phi \left(\C_{i-1}, \bm x\right)$ otteniamo
    $$ \phi \left(\C_{i-1}, \bm a\right) \leq 2 \left(\phi \left(\C_{i-1}, \bm x\right) + \| \bm a - \bm x \|^2 \right) $$

    Facendo la media di questa disuguaglianza su tutti $\bm x \in A$, otteniamo che
    $$ \phi \left(\C_{i-1}, \bm a\right) \leq \frac{2}{|A|} \sum_{\bm a \in A} \left(\phi \left(\C_{i-1}, \bm x\right) + \| \bm a - \bm x \|^2 \right) $$

    Inoltre, per ogni $\bm x \in \X$
    $$ \phi \left(\C_i, \bm x\right) = \min \left\{\phi \left(\C_{i-1}, \bm x\right), \ \| \bm x - \bm c_i \|^2 \right\} $$

    Di conseguenza, per $\C_i = \C_{i-1} \cup \left\{\bm c_i \right\}$
    \begin{align*}
        \Ex & \left[\phi \left(\C_i, A\right) \mid \bm c_i \in A, \ \C_{i-1} \right] = \sum_{\bm a \in A} \frac{\phi \left(\C_{i-1}, \bm a\right)}{\sum_{\bm x \in A} \phi \left(\C_{i-1}, \bm x\right)}\phi \left(\C_i, A\right) \\
        & \leq \frac{2}{|A|} \sum_{\bm a \in A} \sum_{\bm x \in A} \frac{\phi \left(\C_{i-1}, \bm x\right) + \| \bm a - \bm x \|^2}{\sum_{\bm x' \in A} \phi \left(\C_{i-1}, \bm x'\right)} \sum_{\bm a' \in A} \min \left\{\phi \left(\C_{i-1}, \bm a'\right), \ \| \bm a' - \bm a \|^2 \right\} \\
        & = \frac{2}{|A|} \sum_{\bm a \in A} \frac{\sum_{\bm x \in A} \phi \left(\C_{i-1}, \bm x\right)}{\sum_{\bm x' \in A} \phi \left(\C_{i-1}, \bm x'\right)} \sum_{\bm a' \in A} \min \left\{\phi \left(\C_{i-1}, \bm a' \right), \ \| \bm a' - \bm a \|^2 \right\} \\
        & \qquad + \frac{2}{|A|} \sum_{\bm a \in A} \sum_{\bm x \in A} \frac{\| \bm a - \bm x \|^2}{\sum_{\bm x' \in A} \phi \left(\C_{i-1}, \bm x \right)} \sum_{\bm a' \in A} \min \left\{\phi \left(\C_{i-1}, \bm a'\right), \ \| \bm a' - \bm a \|^2 \right\} \\
...
Un cluster $A \in \C^\ast$ è scoperto un $C_i$ se $A \cap \left\{\bm c_1, \dots, \bm c_i\right\} = \emptyset$. Il Lemma \ref{lemma:kmpp-hell} mostra che paghiamo $\O (\Opt)$ per ogni cluster ottimale che copriamo. Questo giustifica le seguenti assunzioni, ponendo il costo di ogni cluster ottimale a 1, e paghiamo 1 per ogni cluster coperto e $L$ per ogni cluster ottimale che rimane scoperto. \\
...
\begin{proof}
    ...
    Sia $N_i$ il numero di cluster scoperti in $\C_i$. Secondo le assunzioni, $\Phi (\C_i) = N_i L + \left(k - N_i\right)$.
    ...
    $$ \Pr \left(\exists A \in \C^\ast : \bm C_{i+1} \in A \wedge A \cap \left\{\bm c_1, \dots, \bm c_i\right\} = \emptyset \mid \C_i \right) = \frac{N_i L}{N_i L + \left(k - N_i\right)} \geq \frac{(k - i)L}{(k-i) L + i} $$
    dove abbiamo usato $N_i \geq k - i$.

    Se $\bm c_{i+1}$ non copre nessun A precedentemente scoperto in $\C_i$ (che accade con probabilità $1 - p_{i+1}$), allora $\Phi \left(\C_{i+1}\right) \leq \Phi \left(\C_i\right)$. Viceversa, se $\bm c_{i+1}$ copre un quale $A$ precedentemente non coperto in $\C_i$ (il che accade con probabilità $p_{i+1}$), allora $\Phi \left(\C_{i+1}\right) = \Phi \left(\C_i\right) - L + 1 = \Phi (\C_i) - (L - 1)$. Di conseguenza
    \begin{align*}
    ...
    \end{align*}
    dove abbiamo usato il bound sulla somma  armonica $1 + \frac{1}{2} + \frac{1}{3} + \dots + \frac{1}{k} \leq 1 + \ln k$. La dimostrazione si conclude notando che, sotto le nostre assunzioni, $\Opt = \Phi \left(\C^\ast \right) = k$.
\end{proof}
