...
L'algoritmo $k$-means$++$ (Algoritmo \ref{alg:kmpp}) definisce la probabilità di scegliere un punto $\bm x$ come nuovo centro in maniera proporzionale alla distanza dai centri già scelti, più un punto è lontano dai centri attuali (valore al numeratore), più alta sarà la probabilità che venga scelto.
...
\begin{proof}
    ...
    \begin{align*}
        ...
        & = \frac{1}{|A|} \sum_{\bm a \in A} \left(|A| \| \bm a - \bm \mu \|^2 + \sum_{\bm x \in A} \| \bm x - \bm \mu \|^2\right) && (\bm \mu \text{ centroide di } A) \\
	...
	di conseguenza
	\[ \phi \left(\C_i, A \right) = \sum_{\bm a \in A} \min \left\{ \phi \left(\C_{i-1}, \bm a\right), \ \| \bm a - \bm \mu_A \|^2 \right\} \tag*{$(\ast \ast)$} \]
