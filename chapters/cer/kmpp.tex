...
Un cluster $A \in \C^\ast$ è scoperto in $\C_i$ se $A \cap \left\{\bm c_1, \dots, \bm c_i\right\} = \emptyset$ (nessuno dei centri selezionati da $\C_i$ appartiene a $A$). Il Lemma \ref{lemma:kmpp-hell} mostra che paghiamo $\O (\Opt)$ per ogni cluster ottimale che copriamo. Questo giustifica le seguenti assunzioni, ponendo il costo di ogni cluster ottimale a 1, e paghiamo 1 per ogni cluster coperto  (ovvero il costo è "basso") e $L$ per ogni cluster ottimale che rimane scoperto (costo "alto"). \\
...
\begin{proof}
    ...
    %TODO: avvisare? Non c'era N_i al numeratore negli appunti
    Per ogni $A$ scoperto, la probabilità che nell'iterazione $i+1$ venga scelto un centro all'interno di $A$ è
    $$ \Pr \left(\bm c_{i+1} \in A \mid \C_i \right) = \frac{\phi \left(\C_i, A\right)}{\Phi \left(\C_i\right)} = \frac{N_i L}{N_i L + \left(k - N_i\right)} $$
    ...
    La dimostrazione si conclude notando che, sotto le nostre assunzioni, $\Opt = \Phi \left(\C^\ast \right) = k$.
\end{proof}
