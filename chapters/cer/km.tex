...
Si noti che ogni punto "costa" come il quadrato della distanza dal centro più vicino. Il $k$-clustering ottimale $\C^\ast$ è una qualsiasi scelta di centri (partizione) tale che minimizzi il costo, ovvero
...
$$ \bm \mu_i = \arg \min_{\bm c} \Ex \left[\|\bm X - \bm c\|^2 \right], \quad \text{dove } \bm X \sim \Nc \left(\bm \mu_i, \sigma_i^2 I \right) \ \text{ e } \ \Ex \left[\Phi\left(\bm \mu_1, \dots, \bm \mu_k\right)\right] \leq \sum_{i = 1}^k \sigma_i^2 $$
...
\begin{proof}
    ...
    Quindi, $\Phi(\C) / \Opt = \Omega (a)$. Al crescere di $n$, $n \rightarrow \infty$, abbiamo che $p_n \rightarrow 1$, con la conseguenza che il caso sfavorevole accade con probabilità arbitrariamente alta.
\end{proof}
...
%    \[ \sum_{\bm x \in C} \|\bm x - \bm c\|^2 = \sum_{\bm x \in C} \|\bm x - \bm \mu \|^2 + |C|\|\bm c - \bm \mu \|^2 \tag*{($\dag$)}\]
