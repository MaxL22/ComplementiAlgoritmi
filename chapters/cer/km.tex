...
In altre parole, $k$-means raggruppa i punto basandosi sulla distanza euclidea dal centroide, e tale distanza "ragiona" per sfere, raggruppando tutto ciò che è entro un certo raggio. Se i punti avessero distribuzioni ellittiche/irregolari $k$-means li taglierebbe in modo innaturale in quanto cerca delle sfere.
...
Il tempo per ogni iterazione dell'algoritmo è nell'ordine di $\O(nkd)$ (dato che ognuno degli $n$ punti va confrontano con ognuno dei $k$ centroidi tramite un calcolo su $d$ dimensioni). Si possono usare proiezioni casuali per mappare $\X$ a $\R^N$ con $N = \Theta (\ln n)$, risultando in un valore di \Opt\ moltiplicato per una costante. Questo riduce il tempo per iterazione a $\O (nk \ln n)$. Il caso peggiore per quanto riguarda il numero di iterazioni dell'algoritmo, purtroppo, è $2^{\Omega (\sqrt{n})}$.
...
\begin{proof}
    ...
    Si noti che $\psi \left(C_1, \dots, C_k, \bm c_1, \dots, \bm c_k \right) \geq \psi \left(C_1', \dots, C_k', \bm c_1, \dots, \bm c_k \right)$ dato che Linea 4 assegna ogni punto al centro più vicino (stiamo considerando i centroidi "vecchi" con il nuovo assegnamento dei punti, prima di ricalcolare i centroidi).
